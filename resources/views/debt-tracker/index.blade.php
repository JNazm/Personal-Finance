<x-app-layout>
    ...
    <div class="py-10">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-sm rounded-2xl p-6">
                <h3 class="text-lg font-semibold text-gray-800 mb-4">Add New Debt</h3>
                <form method="POST" action="{{ route('debt-tracker.store') }}">
                    @csrf
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" required
                                ...
                                placeholder="e.g. Car Loan" value="{{ old('name') }}">
                    ...
                    <div class="px-6 pt-5 pb-3">
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <form method="POST" action="{{ route('debt-tracker.destroy', $debt) }}" onsubmit="return confirm('Delete this debt?')">
                                    @csrf @method('DELETE')
                                    ...
                                    @foreach($debt->payments->sortBy('month_index') as $payment)
                                        ...
                                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                                            ...
                                            <td class="py-3 px-4 text-center">
                                                <button type="button"
                                                    data-toggle-url="{{ route('debt-tracker.toggle', [$debt, $payment->month_index]) }}"
                                                    ...
                                                    @if($payment->is_paid)
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-white check-icon" viewBox="0 0 20 20" fill="currentColor">
                                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                                                        </svg>
                                                    @else
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-white check-icon hidden" viewBox="0 0 20 20" fill="currentColor">
                                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                                                        </svg>
                                                    @endif
                                                </button>
            ...
            @empty
                <div class="bg-white shadow-sm rounded-2xl p-10 text-center text-gray-400">
                    <p class="text-lg">No debts yet. Add one above!</p>
                </div>
            @endforelse
    ...
    <script>
        ...
        function calcPayment() {
            ...
            const el  = document.getElementById('payment_per_month');
            el.value  = (amt > 0 && mo > 0) ? 'RM ' + (amt / mo).toFixed(2) : 'RM 0.00';
        }
